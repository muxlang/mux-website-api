...
First changelog for `mux-website-api` as a standalone, independently versioned
repo, extracted from the former monorepo. The playground API is deployed, not
published; its version is tracked by git tag, and `ARG MUX_VERSION` in the
`Dockerfile` selects the compiler release it runs.

### Added
- **API test suite + coverage**: Added the test suite and wired coverage reporting
  into SonarCloud.
- **Compiler-main canary**: A non-gating CI job that builds the playground against
  the compiler's `main` to catch breaks early (#2).

### Changed
- **Serve compiler 0.5.0**: `ARG MUX_VERSION` 0.4.1 -> 0.5.0, so the playground
  downloads and runs the 0.5.0 compiler release. (Deploy only after the compiler
  v0.5.0 GitHub Release exists.)
